...
while running:
            ...
            if weapon_rect.colliderect(ball_rect):
                ...
                if ball_img_idx < 3:
                    ...
                    balls.append({
                        ...
                        "to_y" : -6, # y축 이동방향 ( -6 : 처음에 공이 생길 때 살짝 올라갔다가 내려오는 정도의 속도)
                        "init_spd_y" : ball_speed_y[ball_img_idx + 1]}) # y 최초 속도

                    # 오른쪽으로 튕겨나가는 다음 단계의 공
                    balls.append({
                        "pos_x" : ball_pos_x + (ball_width / 2) - (small_ball_width / 2), # 공 x 좌표
                        "pos_y" : ball_pos_y + (ball_height / 2) - (small_ball_height / 2), # 공 y 좌표
                        "img_idx" : ball_img_idx + 1, # 공의 이미지 index
    ...
    # 경과 시간 계산
    elapsed_time = (pygame.time.get_ticks() - start_ticks) / 1000 # ms   -> s , 현재 시간 - 스타트 틱
...
msg_rect = msg.get_rect(center=(int(screen_width / 2), int(screen_height / 2))) # 화면 중앙에 노출
screen.blit(msg, msg_rect)
pygame.display.update() # 위 코드를 화면에 반영
